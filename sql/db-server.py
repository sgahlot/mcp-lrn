...
async def get_all_employees() -> List[Dict]:
  ...
  try:
    async with aiosqlite.connect(DB_PATH) as conn:
      logger.info(f"Getting all employees from {DB_PATH} - using table {TABLE_NAME}")
    ...
    return [
      {
        "error": f"Encountered an unexpected error while getting all the employees: {ex}\nDB_PATH used={DB_PATH}"
      }
    ]
...
async def get_employee(employee_id: int) -> Optional[Dict]:
  ...
  async with aiosqlite.connect(DB_PATH) as conn:
    logger.info(f"Getting employee {employee_id} from {DB_PATH} - using table {TABLE_NAME}")
...
async def delete_employee(employee_id: int) -> bool:
  ...
  async with aiosqlite.connect(DB_PATH) as conn:
    try:
      logger.info(f"Deleting employee {employee_id} from {DB_PATH} - using table {TABLE_NAME}")
...
async def init_db(base_db_dir: str = "") -> str:
  ...
  try:
    ...
    logger.info(f"Initializing the database... {base_db_dir}")
